document.addEventListener("DOMContentLoaded", async () => {
  ...
  const targetGus = ["사하구", "부산진구", "동래구", "영도구"];
 ...
 * - 검색어 뒤에 "포토 스팟"을 자동으로 추가합니다. (app.py에서 처리)
 */
...
const handleSearch = async () => {
  ...
  // 로딩 중 표시 및 검색 영역 활성화
  resultsQuerySpan.textContent = `"${query} 포토 스팟" (검색 중...)`;
  ...
  try {
    ...
    const response = await fetch(
      `/api/naver-photo?q=${encodeURIComponent(query)}`
    );
    ...
    // 검색 결과 표시 업데이트
    resultsQuerySpan.textContent = `"${query} 포토 스팟" (${data.items.length}건)`;
    ...
    resultsQuerySpan.textContent = `"${query} 포토 스팟" (검색 실패)`;
  }
};
