...
window.handleSearch = async () => {
    ...
    if (!searchInput || !searchResultsDiv || !resultsQuerySpan || !resultsListDiv) {
    ...
    resultsListDiv.innerHTML = '<div style="padding: 12px; text-align: center; color: #808080;">네이버 블로그 검색 결과를 불러오는 중...</div>';
    searchResultsDiv.hidden = false;

    try {
        // query 파라미터로 사용자가 입력한 값만 보냅니다.
        const response = await fetch(`/api/naver-walk?q=${encodeURIComponent(query)}`);
        
        if (!response.ok) {
            const errorJson = await response.json().catch(() => ({ error: `HTTP ${response.status} 오류` }));
            throw new Error(errorJson.error || `서버에서 알 수 없는 오류 발생 (Status: ${response.status})`);
        }
        ...
        if (data.items.length === 0) {
            resultsListDiv.innerHTML = '<div style="padding: 12px; text-align: center; color: #808080;">검색 결과가 없습니다.</div>';
            return;
        }
            ...
            <div class="results-item">
                <a href="${item.link || '#'}" target="_blank" rel="noopener noreferrer" style="display: block;">
                    ...
                    <div class="results-meta">
                        ${item.description} <span style="color:#559DD5; margin-left: 8px;">(by ${item.bloggername || '블로그'})</span>
                    </div>
                </a>
            </div>
        `).join('');
        ...
        if (errorMsg.includes("NAVER API 키가 없습니다")) {
             errorMsg = "NAVER API 키(Client ID/Secret)가 서버에 설정되지 않았습니다. app.py 환경 설정을 확인하세요.";
        }
